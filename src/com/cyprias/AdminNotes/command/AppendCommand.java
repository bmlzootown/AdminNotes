...
public class AppendCommand implements Command {
	...
	public void listCommands(CommandSender sender, List<String> list) {
		if (Plugin.hasPermission(sender, Perm.APPEND))
			list.add("/%s append - Append a message to your note.");
	}


	public boolean execute(final CommandSender sender, org.bukkit.command.Command cmd, String[] args) {
		if (!Plugin.checkPermission(sender, Perm.APPEND)) {
			return false;
		}
		if (args.length < 2){
		...
		try {
			...
			text += " " + ChatColor.stripColor(Plugin.getFinalArg(args, 1));
	...
	public void getCommands(CommandSender sender, org.bukkit.command.Command cmd) {
		ChatUtils.sendCommandHelp(sender, Perm.APPEND, "/%s append <id> <message>", cmd);
	}
